...
Completed uploads atomically persist a preparation job and dispatch intent. Each job uses a stable Workflow ID, one running slot per candidate, bounded retries, a five-minute deadline, and revision/lifecycle checks before publication. The local Workflow verifies actual WAV bytes and stores a SHA-256/metadata checkpoint. Polling reads progress without creating jobs. WAV validation itself calls no transcription or AI service; configured downstream processing can make paid calls. The shared processing ledger accounts for paid stages using integer microdollars, with a $50 reservation cap and unknown charges retained.

Preview uses separate app, job, and private media Workers. The job Worker is configured in `wrangler.jobs.jsonc`; changes to it require a separate deployment from the app. See [preview evidence](docs/verification/multipart-preview.md) for deployed versions, observations, and remaining acceptance. Production release remains separate.
...
The local adapter makes no paid calls. Its bounded scratch files are removed after each operation, including cancellation; a new exclusive service startup removes files left by a crash. Review deletion cancels extraction and repeatedly removes both original keys and all audio attempt keys to catch late writes. The hosted media runtime has [preview format and duration-boundary evidence](docs/verification/media-boundary-preview.md). Representative exported interview acceptance remains deferred; synthetic codec fixtures do not establish quality on candidate recordings.

OpenAI local setup: set `OPENAI_API_KEY` in the ignored `.env` file and restart `pnpm dev`. The launcher synchronizes only that server-side key into ignored `.dev.vars` for the background Worker. The key is never passed to the browser. New uploads proceed from preparation to `gpt-4o-transcribe-diarize` with English, `diarized_json`, and automatic internal chunking. Recordings up to 1,400 seconds use one compressed request. Longer recordings use up to three contiguous parts, each at most 1,200 seconds and 15,000,000 compressed bytes. The threshold follows the observed provider rejection documented in [multipart preview verification](docs/verification/multipart-preview.md); it is an application policy, not a promise that all provider limits are independently verified. Each part has its own paid identity and saved receipt. Completed parts are reused during recovery instead of resubmitted.

Initial transcripts and provider responses are private immutable R2 artifacts. Assembly validates continuous part coverage, converts timestamps back to the original recording timeline, and namespaces speaker labels by part. Labels from different provider requests are not assumed to identify the same person. Passages near part boundaries carry an uncertainty marker. The UI preserves approximate timing and overlapping-speech distinctions without inventing confidence or word alignment, and pages through all segments rather than truncating the interview. Confirm the candidate labels in every part before grouping and coaching.

The transcription policy initially reserves $6 per paid attempt, including all parts of a multipart attempt. Eligible multipart retries reserve $2 for each unfinished part. The implementation settles returned input/output token usage at $2.50/$10 per million and retains reservations for unknown usage. This conservative reservation is **not a verified provider-enforced maximum**: the API exposes no documented output-token cap for internally chunked diarization. A verified worst-case bound remains a pilot blocker; the $50 ledger enforces reservations, not an unconditional upstream billing guarantee. An interrupted/ambiguous request is never automatically resubmitted. OpenAI exposes no documented transcription-result retrieval endpoint; retained request IDs and receipts support manual reconciliation, but cannot guarantee recovery of a lost result.

Provider evidence includes a generated 10.28-second two-voice probe and [synthetic hour-long audio/video probes](docs/verification/multipart-preview.md) that produced all three transcription parts and speech near the end. Those hour-long journeys failed later or had attribution/fixture limitations; they do not establish a completed recording-to-saved-preparation flow, continuous transcription accuracy, or reliable diarization. Permissioned recording acceptance and independent quality evaluation remain outstanding. The [hour-pipeline test](docs/verification/hour-pipeline-preview.md) is opt-in; sample-interview-dependent acceptance remains deferred.

Provider retention checked on September 21 against [OpenAI data controls](https://developers.openai.com/api/docs/guides/your-data): `/v1/audio/transcriptions` lists no abuse-monitoring or application-state retention. Grouping/coaching use foreground `/v1/responses` calls with `store: false`; this does not disable abuse-monitoring retention, which defaults to up to 30 days with stated exceptions. The actual project's approved retention controls have not been verified. App-retained R2/D1 artifacts, Workflow state, billing metadata, and platform backups are separate; application deletion does not prove erasure outside its control.

After transcription, listen to the short speaker samples and select every detected label that represents your voice. Confirmation is tied to that immutable transcript and survives reload. Unselected speaker labels and unidentified speech are preserved. Waiting for confirmation makes no paid calls and holds no automatic processing slot. Saving creates a durable continuation intent; account concurrency may keep it visibly queued. The question-grouping stage consumes the confirmed attribution.
