...
test('candidate saves preparation, recovers a conflict, returns in a new session and deletes it',async({page,context})=>{
 ...
 await context.request.post(endpoint,{headers:{origin},data:{jobId:job,version:2,text:'Saved from another tab.'}});await answer.fill('My unsaved draft survives.');await panel.getByRole('button',{name:'Save future answer',exact:true}).click();await expect(panel.getByRole('alert').filter({hasText:'This saved answer changed'})).toBeVisible();await expect(answer).toHaveValue('My unsaved draft survives.');await panel.getByRole('button',{name:'Load latest saved answer'}).click();await expect(answer).toHaveValue('Saved from another tab.');
