...
for(const stage of ['grouping','coaching'] as const)test(`candidate retries ${stage} and returns to the persisted result`,async({page,context})=>{
 test.setTimeout(150000);const origin=testOrigin,email=`recovery-${randomUUID()}@example.com`;
 ...
 try{
  const doc=join(folder,'transcript.json'),seed=join(folder,'seed.sql');writeFileSync(doc,JSON.stringify(transcript),{mode:0o600});execFileSync('pnpm',['exec','wrangler','r2','object','put',`${mediaBucket}/${key}`,...storageFlags,'--file',doc],{stdio:'pipe'});
  ...
  writeFileSync(seed,statements.join('\n'),{mode:0o600});execFileSync('pnpm',['exec','wrangler','d1','execute','DB',...databaseFlags,'--file',seed],{stdio:'pipe'});
