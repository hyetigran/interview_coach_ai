...
OpenAI transcription uses one full-recording request with server-side compression and `chunking_strategy=auto` to retain a common speaker-label namespace. Preparation atomically creates a transcription intent before releasing its account slot. A durable Workflow step claims that intent once; D1 stores submission state and the provider request ID, and R2 stores the immutable provider receipt and normalized transcript. A crash or lost result changes the operation to unknown; it never triggers another paid submission. The bounded local policy reservation and unresolved provider cost ceiling are documented in README and remain pilot acceptance limitations.
