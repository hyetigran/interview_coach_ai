...
Ticket #4's local media adapter is a separate Node.js/FFmpeg process, reached only through an authenticated loopback endpoint by the local Worker. It accepts bounded binary media, disallows network protocols and non-video demuxers, probes actual streams and duration, and extracts one audio track. The Worker registers each derivative implicitly through the retained upload ID (`audio/<upload-id>/<attempt-id>`), verifies it, and publishes only under the active job/revision predicate. Cleanup always covers that derivative prefix, including after deletion or a late write. This is a free local adapter; the separate hosted adapter has partial [preview format and duration evidence](verification/media-boundary-preview.md), with representative recording and full-pipeline acceptance still outstanding. FFmpeg [stream probing](https://ffmpeg.org/ffprobe.html) and [protocol restrictions](https://ffmpeg.org/ffmpeg-protocols.html) inform the process boundary.

OpenAI transcription uses one compressed request up to 1,400 seconds; longer recordings use at most three contiguous parts of at most 1,200 seconds each. Every request uses `chunking_strategy=auto`, but speaker labels are namespaced per part because independent requests do not share speaker identity. Assembly validates full timeline coverage, restores original timestamps, and marks passages near processing cuts as uncertain. Preparation atomically creates a transcription intent before releasing its account slot. D1 stores durable part/submission identities; R2 stores individual provider receipts, aggregate receipts, and the normalized transcript. Reconciliation can publish saved receipts and resume unfinished parts without resubmitting completed parts. Unknown submissions block resubmission and retain their reservations. The policy reservation and unverified upstream cost ceiling remain distinct; see [README](../README.md) and [multipart preview evidence](verification/multipart-preview.md).

Speaker confirmation is a persisted, version-checked prerequisite. The server validates selected labels against the private transcript, atomically stores the selection and pending continuation intent, then dispatches a stable continuation Workflow ID. There is no long-lived event waiter: durable creation/reconciliation handles both confirmation before execution and lost dispatch responses. Workflow payloads contain only a confirmation ID and cannot supply attribution. Resumption checks the saved confirmation, live review, transcript version, deadline and account slot. Waiting costs nothing; downstream paid stages must reserve their own budget before calls.
