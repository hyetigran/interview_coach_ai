...
export function PreparationStatus({ reviewId }: { reviewId: string }) {
  ...
  if (job?.state === 'ready') return <div className="mt-3"><p role="status">Recording prepared</p><audio controls preload="metadata" src={`/api/reviews/${reviewId}/audio`} aria-label="Private interview recording" /><p className="text-sm text-muted-foreground">{Math.round((job.result?.durationMs ?? 0) / 1000)} seconds of verified audio. Transcription is coming in the next update.</p></div>;
