...
export function ReviewContextEditor({reviewId}:{reviewId:string}) {
 ...
 return <details className="my-5 rounded-md border p-4"><summary className="cursor-pointer font-medium">Target role and optional background</summary>
  ...
  <ContextForm key={query.data.revision} reviewId={reviewId} snapshot={query.data} report={setMessage}/>
...
function ContextForm({reviewId,snapshot,report}:{reviewId:string;snapshot:Snapshot;report:(message:string)=>void}) {
 const [value,setValue]=useState(snapshot.context);const cache=useQueryClient();const dirty=JSON.stringify(value)!==JSON.stringify(snapshot.context);
 const save=useMutation({mutationFn:()=>api<Snapshot>(`/api/reviews/${reviewId}/context`,{method:'PUT',body:JSON.stringify({revision:snapshot.revision,context:value})}),onSuccess:data=>{cache.setQueryData(['context',reviewId],data);void cache.invalidateQueries({queryKey:['coaching',reviewId]});void cache.invalidateQueries({queryKey:['review']});report('Context saved. Affected coaching is outdated. Choose Reanalyze coaching to use this selection; saving does not spend processing credits.');}});
 const reanalyze=useMutation({mutationFn:()=>api(`/api/reviews/${reviewId}/coaching`,{method:'POST',body:JSON.stringify({actionId:crypto.randomUUID(),contextRevision:snapshot.revision})}),onSuccess:()=>{void cache.invalidateQueries({queryKey:['coaching',reviewId]});report('Reanalysis requested. Current results are reused when available; new context is processed under the shared allowance.');}});
 ...
 return <form className="space-y-4" onSubmit={event=>{event.preventDefault();save.mutate();}}>
  ...
  <div className="flex gap-3"><Button type="submit" disabled={!dirty||save.isPending}>{save.isPending?'Saving…':'Save context'}</Button><Button type="button" variant="outline" disabled={dirty||save.isPending||reanalyze.isPending} onClick={()=>reanalyze.mutate()}>Reanalyze coaching</Button></div>
